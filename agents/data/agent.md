...
- Read-only access to pipeline state. Write access: append `## Notes` lines to existing card files; new cards via `goal-to-card.mjs` only (never ad-hoc file creates).
